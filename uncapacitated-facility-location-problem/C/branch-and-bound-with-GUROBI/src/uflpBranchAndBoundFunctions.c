...
void gurobiSolver(uflpModel **model)
{
  ...
  error = GRBsetintparam(GRBgetenv(gModel),GRB_INT_PAR_MIPFOCUS, 2); //only for MIP
  //error = GRBsetintparam(GRBgetenv(gModel),GRB_INT_PAR_THREADS, 1);
  error = GRBsetintparam(GRBgetenv(gModel),GRB_INT_PAR_CUTS, 2); //only for MIP
  error = GRBsetintparam(GRBgetenv(gModel),GRB_INT_PAR_PRESOLVE, 1);
